...
import { User, Phone, MapPin, Clock, XCircle, Printer, Loader2, X, Receipt } from "lucide-react";
...
const dialogShellClass = cn(
  ...
  "w-[calc(100%-1.5rem)] max-w-md staff-wide:max-w-xl",
  "max-h-[min(92dvh,calc(100dvh-env(safe-area-inset-top)-env(safe-area-inset-bottom)-0.75rem))]",
  "rounded-2xl sm:rounded-3xl border border-border bg-card shadow-2xl",
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogPortal>
          ...
          <DialogPrimitive.Content className={dialogShellClass}>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPortal>
        <DialogOverlay className="bg-black/75 backdrop-blur-sm" />
        <DialogPrimitive.Content className={dialogShellClass}>
          <div className={cn("shrink-0 px-4 py-3 text-center rounded-t-2xl sm:rounded-t-3xl", modalityBannerClass[modality.tone])}>
          ...
          <DialogPrimitive.Close
            className="absolute right-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-black/20 text-white transition hover:bg-black/35 focus:outline-none focus:ring-2 focus:ring-white/40"
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-4 space-y-4">
              <div className="space-y-1 pr-8">
                <DialogTitle className="flex flex-wrap items-baseline justify-between gap-2 text-left text-lg font-bold">
                  <span className="flex items-center gap-2">
                    <Receipt className="h-5 w-5 shrink-0 text-primary" />
                    {panelT(lang, "order.detail.title", { code: order.order_number })}
                  </span>
              ...
              {order.delivery_street && (
                <div className="flex items-start gap-2 text-sm rounded-xl border p-3">
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  {items.map((it) => {
                    ...
                    return (
                      <li key={it.id} className="rounded-xl border p-2.5 space-y-2">
            ...
            <div className="shrink-0 space-y-2 border-t bg-card px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
              {action && order.status !== "cancelled" && (
                <Button
                  className={cn(
                    "w-full h-11 font-bold touch-action-manipulation",
