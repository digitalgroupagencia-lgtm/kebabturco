  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-2">
        <Loader2 className="w-4 h-4 animate-spin" /> {t("modifier.loading")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 rounded-xl border border-primary/20 bg-primary/5 p-4">
      <div className="flex items-center justify-between gap-2">
        <p className="text-sm font-bold text-foreground">{t("modifier.title")}</p>
        <Link to={nav.admin("modifiers")} className="text-xs font-bold text-primary hover:underline">
          {t("modifier.manage_link")}
        </Link>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label>{t("modifier.product_type")}</Label>
          <Select value={productType} onValueChange={(v) => onProductTypeChange(v as "simple" | "combo")}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="simple">{t("modifier.type.simple")}</SelectItem>
              <SelectItem value="combo">{t("modifier.type.combo")}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        {productType === "combo" && (
          <div>
            <Label>{t("modifier.combo_units")}</Label>
            <Input
              type="number"
              min={1}
              max={20}
      ...
      {groups.length === 0 ? (
        <p className="text-xs text-muted-foreground">{t("modifier.empty_groups")}</p>
      ) : (
        <div className="space-y-2 max-h-56 overflow-y-auto">
          {sortModifierGroups(
            ...
            const displayName =
              g.name?.[lang] || g.name?.pt || g.name?.es || g.name?.en || "";
            return (
              <div key={g.id} className="flex items-center gap-2 rounded-lg border bg-card px-3 py-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {groupKindLabel(g.group_kind as ModifierGroupKind, kindLang)}
                    {g.is_required ? t("modifier.required") : ""}
                  </p>
                </div>
                {linked && productType === "combo" && (
                  <label
                    className="flex items-center gap-1 text-[10px] font-bold shrink-0"
                    title={t("modifier.per_unit.title")}
                  >
      ...
      {productType === "combo" && links.length > 0 && (
        <p className="text-[11px] text-muted-foreground leading-snug">{t("modifier.combo_hint")}</p>
      )}
    </div>
  );
}
