...
const badgeBase =
  "flex h-11 w-full min-w-0 items-center justify-center gap-2 rounded-xl px-2.5 active:scale-[0.98] transition-transform touch-manipulation";
...
const InstallAppButton = ({ lang = "es", variant = "primary" }: Props) => {
  ...
  const linkClass = isSubtle
    ? `${badgeBase} border border-border/40 bg-background text-foreground`
    : `${badgeBase} bg-black text-white shadow-[0_6px_18px_-8px_rgba(0,0,0,0.55)]`;

  const iconClass = isSubtle ? "h-5 w-5 shrink-0" : "h-5 w-5 shrink-0";
  const kickerClass = isSubtle
    ? "text-[8px] uppercase leading-none opacity-70"
    : "text-[8px] uppercase leading-none opacity-80";
  const titleClass = isSubtle
    ? "text-[11px] font-bold leading-tight truncate max-w-full"
    ...
    <div className="w-full max-w-[340px] mx-auto px-1">
      <div className="grid grid-cols-2 gap-2.5 w-full items-stretch">
        <a
          ...
          className={linkClass}
        >
          <svg viewBox="0 0 384 512" aria-hidden className={iconClass} fill="currentColor">
          ...
          <div className="flex min-w-0 flex-col items-start justify-center">
            <span className={kickerClass}>Download</span>
            <span className={titleClass}>{t.appStore}</span>
          </div>
        </a>
        ...
        <a
          ...
          className={linkClass}
        >
          <svg viewBox="0 0 512 512" aria-hidden className={iconClass}>
          ...
          <div className="flex min-w-0 flex-col items-start justify-center">
            ...
            <span className={titleClass}>{t.playStore}</span>
          </div>
        </a>
      </div>
    </div>
