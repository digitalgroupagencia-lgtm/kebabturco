          ...
          {tabs.map(({ id, label, icon: Icon, onClick }) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={onClick}
                className="relative flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 touch-manipulation"
              ...
              >
                <span
                  className={`relative flex items-center justify-center rounded-full transition-all ${
                    active
                      ? "h-11 w-11 bg-gradient-primary text-primary-foreground shadow-primary"
                      : "h-9 w-9 text-muted-foreground"
                  }`}
                >
                  <Icon className={`${active ? "h-[22px] w-[22px]" : "h-5 w-5"}`} strokeWidth={active ? 2.25 : 2} />
                  {showCartBadge && (
                    <span className="absolute -right-0.5 -top-0.5 flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-primary px-0.5 text-[7px] font-black text-primary-foreground">
                      {totalItems > 99 ? "99+" : totalItems}
                    </span>
                  )}
                </span>
                <span
                  className={`max-w-[76px] truncate text-[10px] font-bold leading-none ${
