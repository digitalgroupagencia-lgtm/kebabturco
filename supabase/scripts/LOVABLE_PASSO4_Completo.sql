-- PASSO 4 — Query NOVA, depois do Passo 3
-- Funções, regras de acesso e entregadores
-- Usa extensions.crypt (pgcrypto activado no Passo 2)
...
SELECT 'upsert_staff_access_pin' AS funcao,
  EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'upsert_staff_access_pin') AS instalada;

SELECT 'verify_staff_access_pin' AS funcao,
  EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'verify_staff_access_pin') AS instalada;

SELECT 'lookup_staff_user_by_email' AS funcao,
  EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'lookup_staff_user_by_email') AS instalada;

SELECT 'staff_access_pins' AS item,
  EXISTS (SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'staff_access_pins') AS instalada;
