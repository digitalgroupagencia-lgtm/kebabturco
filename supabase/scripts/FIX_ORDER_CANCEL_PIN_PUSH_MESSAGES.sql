-- Correções: PIN no cancelamento, push personalizado, admin PIN 4321.
-- Corre no SQL Editor do Supabase (projeto kvpssbhclafoymhecmuk).
-- Conteúdo = migration 20260707210000 + PIN admin.
...
-- PIN 4321 para admin master (bunitotrader@gmail.com)
DO $$
DECLARE
  v_user_id uuid;
  v_role_id uuid;
  ...
  v_hash text;
BEGIN
  SELECT id INTO v_user_id FROM auth.users WHERE lower(email) = 'bunitotrader@gmail.com' LIMIT 1;
  IF v_user_id IS NULL THEN
    RAISE NOTICE 'Utilizador admin não encontrado — ajuste o email no script.';
    RETURN;
  END IF;

  SELECT ur.id, ur.store_id
  INTO v_role_id, v_store_id
  FROM public.user_roles ur
  ...
  IF v_role_id IS NULL OR v_store_id IS NULL THEN
    SELECT ur.id, ur.store_id
    INTO v_role_id, v_store_id
    FROM public.user_roles ur
    WHERE ur.user_id = v_user_id
  ...
  IF v_role_id IS NULL THEN
    RAISE NOTICE 'Perfil admin sem loja — verifique user_roles.';
    RETURN;
  END IF;

  BEGIN
    v_hash := extensions.crypt('4321', extensions.gen_salt('bf'));
  EXCEPTION
    WHEN undefined_function OR invalid_schema_name THEN
      v_hash := crypt('4321', gen_salt('bf'));
  END;

  INSERT INTO public.staff_access_pins (store_id, user_id, user_role_id, pin_hash, is_active)
  VALUES (v_store_id, v_user_id, v_role_id, v_hash, true)
  ON CONFLICT (user_role_id) DO UPDATE SET
    pin_hash = EXCLUDED.pin_hash,
    is_active = true,
    updated_at = now();

  RAISE NOTICE 'PIN 4321 definido para admin em store %', v_store_id;
END $$;
