-- URGENTE: corrige pagamento em efectivo / Bizum / cartão no cliente
-- Erro: function public.add_loyalty_stamp(uuid, text, uuid) is not unique
...
-- Confirma que só existe uma versão
SELECT p.proname, pg_get_function_identity_arguments(p.oid) AS args
FROM pg_proc p
JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'public' AND p.proname = 'add_loyalty_stamp';
