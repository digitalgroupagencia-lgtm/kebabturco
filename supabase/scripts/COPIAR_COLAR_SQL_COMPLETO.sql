-- =============================================================================
-- KEBAB TURCO — SQL COMPLETO (copiar e colar UMA VEZ no SQL Editor da Lovable Cloud)
-- Inclui: política de pagamento, QR mesas, impressão única, caixa, diagnóstico operacional.
-- Idempotente: pode correr de novo sem estragar dados.
-- =============================================================================
...
-- Revogar confirmação de pagamento anónima (só via edge function verificada)
REVOKE EXECUTE ON FUNCTION public.confirm_order_payment(text, text) FROM anon;
...
CREATE OR REPLACE FUNCTION public.create_customer_order(
  ...
  _delivery_street text DEFAULT NULL,
  _delivery_number text DEFAULT NULL,
  _delivery_complement text DEFAULT NULL,
  ...
  _delivery_zone_name text DEFAULT NULL,
...
BEGIN
  ...
  IF _order_type = 'delivery' THEN
    ...
    IF _delivery_zone_id IS NOT NULL THEN
      SELECT min_order INTO v_zone_min
  ...
  IF _coupon_id IS NOT NULL AND COALESCE(_discount_amount, 0) > 0 THEN
    ...
    WHERE id = _coupon_id AND store_id = _store_id AND is_active = true;
    IF NOT FOUND THEN
...
-- Taxa de serviço online visível no checkout + repasse Connect (restaurante recebe valor do pedido)

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS online_service_fee_cents integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS stripe_connect_account_id text;

COMMENT ON COLUMN public.orders.online_service_fee_cents IS 'Taxa de serviço/pagamento online cobrada ao cliente (€1 plataforma + Stripe)';
COMMENT ON COLUMN public.orders.processing_fee_cents IS 'Alias histórico = online_service_fee_cents após liquidação';
COMMENT ON COLUMN public.orders.stripe_connect_account_id IS 'Conta Stripe Connect usada no pagamento';
...
-- create_customer_order: breakdown financeiro
CREATE OR REPLACE FUNCTION public.create_customer_order(
  _store_id uuid,
  _order_type text,
  _items jsonb,
  ...
  _net_to_store_cents integer DEFAULT NULL,
  _stripe_connect_account_id text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_session uuid;
...
-- =============================================================================
-- STRIPE CONNECT — ambiente teste/produção por loja (OBRIGATÓRIO para modo teste)
-- =============================================================================
