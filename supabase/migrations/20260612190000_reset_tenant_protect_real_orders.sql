-- Só apaga vendas reais se _include_real_orders = true (evita apagar pagamentos Bizum/cartão por engano).
...
CREATE OR REPLACE FUNCTION public.reset_tenant_data(
  ...
  _include_real_orders BOOLEAN DEFAULT false
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF _reset_orders THEN
    ...
    WHERE store_id = ANY(v_store_ids)
      AND (_include_real_orders OR is_test = true);

    IF v_order_ids IS NOT NULL AND array_length(v_order_ids, 1) > 0 THEN
      WITH d AS (
        DELETE FROM public.coupon_redemptions WHERE order_id = ANY(v_order_ids) RETURNING 1
    ...
    IF _include_real_orders THEN
      WITH d AS (DELETE FROM public.payment_gateway_logs WHERE store_id = ANY(v_store_ids) RETURNING 1)
      SELECT COUNT(*) INTO v_gateway_logs FROM d;

      WITH d AS (DELETE FROM public.payment_gateway_transactions WHERE store_id = ANY(v_store_ids) RETURNING 1)
      SELECT COUNT(*) INTO v_gateway_tx FROM d;

      WITH d AS (DELETE FROM public.store_payment_ledger WHERE store_id = ANY(v_store_ids) RETURNING 1)
      SELECT COUNT(*) INTO v_ledger FROM d;
    ELSE
      WITH d AS (
        DELETE FROM public.store_payment_ledger
        WHERE order_id = ANY(COALESCE(v_order_ids, ARRAY[]::uuid[]))
        RETURNING 1
      ) SELECT COUNT(*) INTO v_ledger FROM d;
    END IF;
    ...
    IF v_order_ids IS NOT NULL AND array_length(v_order_ids, 1) > 0 THEN
      ...
      WITH d AS (DELETE FROM public.orders WHERE id = ANY(v_order_ids) RETURNING 1)
      SELECT COUNT(*) INTO v_orders FROM d;
    END IF;
  END IF;

  IF _reset_cash AND _include_real_orders THEN
  ...
  RETURN jsonb_build_object(
    'success', true,
    'include_real_orders', _include_real_orders,
    'deleted', jsonb_build_object(
      'orders', v_orders,
      'cash', v_cash,
      'stock', v_stock,
