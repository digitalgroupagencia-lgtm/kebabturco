-- Customer native push (iOS/Android App Store): marketing + order status via APNs/FCM.

DROP FUNCTION IF EXISTS public.register_native_push_subscription(uuid, text, text, text);
...
GRANT EXECUTE ON FUNCTION public.register_native_push_subscription(uuid, text, text, text, uuid) TO anon, authenticated;
