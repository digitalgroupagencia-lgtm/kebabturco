-- Checkout público: clientes anónimos precisam saber se recebimentos Stripe estão activos
-- (colunas Stripe em stores não são legíveis por anon).
...
COMMENT ON FUNCTION public.get_store_checkout_stripe_profile(uuid) IS
  'Perfil Stripe mínimo para checkout do totem — sem dados sensíveis extra.';

-- Gandia (Kebab Turco): conta Connect live já provisionada na Stripe.
