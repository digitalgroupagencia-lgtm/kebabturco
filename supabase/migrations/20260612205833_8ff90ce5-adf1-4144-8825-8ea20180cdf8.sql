...
AS $$
  SELECT CASE
      ...
      WHEN p.payment_method IN ('card'::public.payment_method, 'bizum'::public.payment_method, 'apple_pay'::public.payment_method, 'google_pay'::public.payment_method) THEN false
...
CREATE TRIGGER orders_staff_push_after_insert
  AFTER INSERT ON public.orders FOR EACH ROW EXECUTE FUNCTION public.trg_orders_staff_push();

DROP TRIGGER IF EXISTS orders_staff_push_after_update ON public.orders;
CREATE TRIGGER orders_staff_push_after_update
  AFTER UPDATE OF payment_status, status ON public.orders FOR EACH ROW EXECUTE FUNCTION public.trg_orders_staff_push();

CREATE OR REPLACE FUNCTION public.record_payment_settlement(
  _stripe_payment_intent_id text, _platform_fee_cents integer, _stripe_fee_cents integer,
  _processing_fee_cents integer, _net_to_store_cents integer,
  _online_service_fee_cents integer DEFAULT NULL, _payment_method text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE v_order public.orders%ROWTYPE; v_service_fee integer; v_restaurant_gross integer; v_pm public.payment_method;
BEGIN
  SELECT * INTO v_order FROM public.orders WHERE stripe_payment_intent_id = _stripe_payment_intent_id LIMIT 1;
  ...
  IF v_order.payment_status = 'paid'::public.payment_status THEN
    RETURN jsonb_build_object('success', true, 'already_paid', true, 'order_id', v_order.id, 'order_number', v_order.order_number, 'store_id', v_order.store_id);
  END IF;
  v_service_fee := COALESCE(_online_service_fee_cents, _processing_fee_cents, 0);
  v_restaurant_gross := COALESCE(_net_to_store_cents,
  ...
  UPDATE public.orders SET
    ...
    updated_at = now()
  WHERE id = v_order.id;
  INSERT INTO public.store_payment_ledger (store_id, order_id, entry_type,
    gross_cents, platform_fee_cents, stripe_fee_cents, processing_fee_cents, net_cents,
    stripe_payment_intent_id, description)
  VALUES (v_order.store_id, v_order.id, 'order_payment',
    v_restaurant_gross, COALESCE(_platform_fee_cents, 0), COALESCE(_stripe_fee_cents, 0),
    v_service_fee, v_restaurant_gross, _stripe_payment_intent_id,
    'Pedido #' || v_order.order_number)
  ON CONFLICT (stripe_payment_intent_id) WHERE stripe_payment_intent_id IS NOT NULL DO NOTHING;
  RETURN jsonb_build_object('success', true, 'order_id', v_order.id, 'order_number', v_order.order_number, 'store_id', v_order.store_id);
END; $$;

CREATE OR REPLACE FUNCTION public.record_payment_failure(
  _stripe_payment_intent_id text, _failure_code text DEFAULT NULL, _failure_message text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
...
BEGIN
  ...
  IF v_order.id IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'order_not_found'); END IF;
  IF v_order.payment_status = 'paid'::public.payment_status THEN
    RETURN jsonb_build_object('success', true, 'already_paid', true, 'order_id', v_order.id, 'order_number', v_order.order_number);
  END IF;
  UPDATE public.orders SET
    ...
    END,
    updated_at = now()
  WHERE id = v_order.id AND payment_status = 'pending'::public.payment_status;
  RETURN jsonb_build_object('success', true, 'order_id', v_order.id, 'order_number', v_order.order_number, 'failure_code', _failure_code);
END; $$;
...
GRANT SELECT ON public.staff_google_pending TO authenticated;
GRANT ALL ON public.staff_google_pending TO service_role;

CREATE INDEX IF NOT EXISTS staff_google_pending_store_status_idx
  ON public.staff_google_pending (store_id, status) WHERE status = 'pending';
...
CREATE POLICY "Managers view store google pending"
  ON public.staff_google_pending FOR SELECT TO authenticated
  USING (
...
AS $$
  SELECT EXISTS (SELECT 1 FROM auth.identities i WHERE i.user_id = _user_id AND i.provider = 'google');
$$;
...
AS $$
DECLARE v_uid uuid := auth.uid(); v_email text; v_name text; v_tenant_id uuid;
  v_role public.app_role; v_pending public.staff_google_pending%ROWTYPE;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'Sessão inválida'; END IF;
  IF _store_id IS NULL THEN RAISE EXCEPTION 'Loja inválida'; END IF;
  IF NOT public.user_has_google_identity(v_uid) THEN
    RAISE EXCEPTION 'Este acesso é só para quem entrou com Google';
  END IF;
  SELECT s.tenant_id INTO v_tenant_id FROM public.stores s WHERE s.id = _store_id AND s.is_active = true LIMIT 1;
  IF v_tenant_id IS NULL THEN RAISE EXCEPTION 'Loja não encontrada'; END IF;
  ...
  WHERE id = v_uid;
  SELECT ur.role INTO v_role FROM public.user_roles ur WHERE ur.user_id = v_uid AND ur.store_id = _store_id LIMIT 1;
  IF v_role IS NOT NULL THEN RETURN jsonb_build_object('status', 'active', 'role', v_role::text); END IF;
  SELECT * INTO v_pending FROM public.staff_google_pending p WHERE p.user_id = v_uid AND p.store_id = _store_id LIMIT 1;
  IF v_pending.id IS NOT NULL AND v_pending.status = 'rejected' THEN RETURN jsonb_build_object('status', 'rejected'); END IF;
...
RETURNS TABLE(id uuid, user_id uuid, email text, full_name text, created_at timestamptz)
...
BEGIN
  ...
  WHERE p.store_id = _store_id AND p.status = 'pending'
  ORDER BY p.created_at DESC;
END; $$;

CREATE OR REPLACE FUNCTION public.add_team_member_to_store(
  _user_id uuid, _role public.app_role, _store_id uuid, _tenant_id uuid
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE v_role_id uuid;
BEGIN
  IF _user_id IS NULL OR _store_id IS NULL OR _tenant_id IS NULL THEN
    RAISE EXCEPTION 'Dados incompletos para adicionar membro'; END IF;
  IF _role = 'admin_master'::public.app_role THEN
    RAISE EXCEPTION 'Papel inválido para a equipa da loja'; END IF;
  IF NOT public.user_manages_store_team(_store_id)
     AND NOT public.has_role(auth.uid(), 'admin_master'::public.app_role) THEN
    RAISE EXCEPTION 'Sem permissão para adicionar membros a esta loja'; END IF;
  IF EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = _user_id AND ur.store_id = _store_id) THEN
    RAISE EXCEPTION 'Esta pessoa já faz parte da equipa desta loja.'; END IF;
  INSERT INTO public.user_roles (user_id, role, store_id, tenant_id)
  VALUES (_user_id, _role, _store_id, _tenant_id) RETURNING id INTO v_role_id;
  DELETE FROM public.staff_google_pending WHERE user_id = _user_id AND store_id = _store_id;
  RETURN v_role_id;
END; $$;

CREATE OR REPLACE FUNCTION public.approve_staff_google_pending(
  _pending_id uuid, _role public.app_role,
  _full_name text DEFAULT NULL, _preferred_language text DEFAULT 'es'
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, auth
AS $$
DECLARE v_row public.staff_google_pending%ROWTYPE; v_role_id uuid;
BEGIN
  IF _pending_id IS NULL THEN RAISE EXCEPTION 'Pedido inválido'; END IF;
  IF _role = 'admin_master'::public.app_role THEN
    RAISE EXCEPTION 'Papel inválido para a equipa da loja'; END IF;
  SELECT * INTO v_row FROM public.staff_google_pending WHERE id = _pending_id AND status = 'pending' LIMIT 1;
  IF v_row.id IS NULL THEN RAISE EXCEPTION 'Pedido não encontrado ou já tratado'; END IF;
  IF NOT public.user_manages_store_team(v_row.store_id)
     AND NOT public.has_role(auth.uid(), 'admin_master'::public.app_role) THEN
    RAISE EXCEPTION 'Sem permissão para aprovar este pedido'; END IF;
  v_role_id := public.add_team_member_to_store(v_row.user_id, _role, v_row.store_id, v_row.tenant_id);
  ...
  RETURN jsonb_build_object('success', true, 'user_role_id', v_role_id, 'user_id', v_row.user_id, 'role', _role::text);
END; $$;

CREATE OR REPLACE FUNCTION public.reject_staff_google_pending(_pending_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, auth
AS $$
DECLARE v_row public.staff_google_pending%ROWTYPE;
BEGIN
  SELECT * INTO v_row FROM public.staff_google_pending WHERE id = _pending_id AND status = 'pending' LIMIT 1;
  IF v_row.id IS NULL THEN RAISE EXCEPTION 'Pedido não encontrado ou já tratado'; END IF;
  IF NOT public.user_manages_store_team(v_row.store_id)
     AND NOT public.has_role(auth.uid(), 'admin_master'::public.app_role) THEN
    RAISE EXCEPTION 'Sem permissão para excluir este pedido'; END IF;
  UPDATE public.staff_google_pending SET status = 'rejected', reviewed_by = auth.uid(),
    reviewed_at = now(), updated_at = now() WHERE id = v_row.id;
