-- Validação server-side do token QR na mesa (dine_in)
...
BEGIN
  ...
  IF _order_type = 'dine_in' THEN
    IF _table_id IS NULL OR NULLIF(btrim(_qr_token), '') IS NULL THEN
    ...
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'Mesa inválida, inactiva ou QR expirado';
    END IF;
