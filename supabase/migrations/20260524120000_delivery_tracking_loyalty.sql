-- Delivery fields, order tracking status, customers, coupons, loyalty, push subscriptions
...
BEGIN
  IF trim(_phone) = '' THEN
    RETURN jsonb_build_object('stamps', 0, 'reward_ready', false);
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'reward_ready', v_acc.stamps >= v_stamps_needed,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT jsonb_build_object(
    ...
    'reward_ready', COALESCE(la.stamps, 0) >= 10
  )
  FROM (SELECT 1) x
...
RETURNS TABLE(
  ...
  delivery_postal_code text, delivery_fee numeric,
  estimated_ready_at timestamptz, discount_amount numeric
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
...
-- Main order creation with delivery, coupon, validation
...
BEGIN
  ...
  IF _order_type = 'delivery' THEN
    ...
    IF _delivery_zone_id IS NOT NULL THEN
      SELECT min_order, delivery_fee INTO v_zone_min, v_zone_fee
  ...
  v_ps := CASE
    WHEN auth.role() = 'anon' THEN 'pending'::public.payment_status
