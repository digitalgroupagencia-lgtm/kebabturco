...
- **🆕 Módulo VENDEDOR (garçom)**: novo papel \`seller\` para funcionários tirarem pedidos pelo celular vinculando mesa + cliente + vendedor.
  - **App do vendedor**: \`/seller\` (mobile-first). Telas: Início (resumo do dia), Mesas abertas, Meus pedidos, Novo pedido. Login redireciona automaticamente — admin_master vai pra \`/admin\`, seller pra \`/seller\`, demais pra \`/panel\`.
  - **Painel do restaurante** ganhou aba **Vendedores** (\`/panel/sellers\`): criar, listar, ver pedidos/faturamento (30d), remover. Bloqueia criação se passar do limite do plano.
  - **Plano comercial expandido** no Admin Master (Assinatura do tenant): adesão, mensalidade base, **vendedores inclusos**, **preço por vendedor extra**, **vendedores liberados**. Mensalidade total é calculada automaticamente: base + (extras × preço). Função SQL \`get_tenant_billing\` retorna o cálculo completo (hook \`useTenantBilling\`).
  - **Banco**: novas tabelas \`tables\`, \`table_sessions\`, \`table_session_customers\`. \`orders\` ganhou \`seller_id\`, \`table_session_id\`, \`table_customer_id\`. Enum \`order_source\` ganhou \`waiter\`. Enum \`app_role\` ganhou \`seller\`.
  - **Edge function \`create-tenant-user\`** aceita role \`seller\` e valida o limite do plano antes de criar.
  - **Próxima entrega** (ainda não pronta): fluxo completo de Novo pedido (mesa→cliente→cardápio→envio à cozinha), detalhe de mesa com fechar cliente individual e fechar mesa inteira (pagamento único OU dividido), impressão dedicada e relatórios por vendedor.
