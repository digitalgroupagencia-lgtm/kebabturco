...
export function isVisitPrintBridgeRequest(req: Request, body?: VisitBridgeBody): boolean {
  if (req.headers.get("x-visit-bridge-token")?.trim()) return true;
  const action = body?.action;
  return action === "heartbeat" || action === "claim" || action === "complete" || action === "fail";
}

export async function handleVisitPrintBridgeRequest(
  req: Request,
  preParsedBody?: VisitBridgeBody,
): Promise<Response | null> {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: visitBridgeCorsHeaders });
  }

  const body = preParsedBody ?? ((await req.json().catch(() => ({}))) as VisitBridgeBody);
  if (!isVisitPrintBridgeRequest(req, body)) return null;

  if (req.method === "GET") {
    return json({ ok: true, service: "visit-print-bridge" });
  }
